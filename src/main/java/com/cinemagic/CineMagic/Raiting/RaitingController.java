...
public class RaitingController {
    ...
    @PostMapping
    public ResponseEntity<Raiting> createRaiting(@RequestBody RatingDTO ratingDTO) {
        Raiting raiting = raitingService.createRaiting(ratingDTO);
        return ResponseEntity.ok(raiting);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Raiting> updateRaiting(@PathVariable Long id, @RequestBody RatingDTO ratingDTO) {
        try {
            Raiting updatedRaiting = raitingService.updateRaiting(id, ratingDTO);
            return ResponseEntity.ok(updatedRaiting);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
